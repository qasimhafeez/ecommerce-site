    ...
    if(isset($_POST['cbArray']))
    {
        ...
        foreach($getBoxArray as $getPostId)
        {
            ...
            switch($choice)
            {
                ...
                //     break;
                case 'delete':
                    $query = "DELETE FROM myproducts WHERE mp_id = {$getPostId} ";
                    $update = mysqli_query($connection, $query);
                    break;
            }
...
<form action="" method="post">    
    <table class="table table-bordered table-hover">
        ...
        <div class="col-xs-4">
           ...
           <a href="myproducts.php?source=add_myproduct" class="btn btn-primary">Add New</a>
        ...
        <?php
            ...
            $query = "SELECT * FROM myproducts ORDER BY mp_id DESC";
        ...
        <?php
                ...
                echo "<td>$p_desc</td>";
                //echo "<td>$p_store</td>";
                $catQ = "SELECT * FROM categories where c_id = {$p_cat}";
                $cat = mysqli_query($connection, $catQ);
            while ($row = mysqli_fetch_assoc($cat)) {
                $cat_name = $row["c_cat"];
                echo '<td>'.$cat_name.'</td>'; 
            }
                echo "<td>$p_date</td>";
                echo "<td><a href='myproducts.php?source=edit_myproduct&p_id={$p_id}'>Edit</a></td>";
                echo "<td><a onClick=\" javascript: return confirm('Are you sure, You want to delete?'); \"
                            href='myproducts.php?delete={$p_id}'>Delete</a></td>";
                echo "</tr>";
            }
        ?>
        ...
        <?php
            if(isset($_GET['delete']))
            {
                ...
                $query = "DELETE FROM myproducts where mp_id = {$delete_post}";
